...
class CurriculumAdd(BaseModel): branch: str; semester: str; subject_code: str; subject_name: str
...
# NEW: Save Fest Registrations to DB
@app.post("/register-fest")
async def register_fest(data: FestRegistration):
    conn = get_db_connection()
    # Check if already registered
    existing = conn.execute("SELECT * FROM fest_registrations WHERE roll_number = ?", (data.roll_number,)).fetchone()
    if existing:
        conn.close()
        return {"success": False, "message": "You are already registered for the Fest!"}
    
    conn.execute("INSERT INTO fest_registrations (roll_number, name, branch, payment_type) VALUES (?, ?, ?, ?)", 
                 (data.roll_number, data.name, data.branch, data.payment_type))
    conn.commit()
    conn.close()
    return {"success": True, "message": "Fest Registration Confirmed!"}
...
# --- ADMIN APIs ---
# NEW: Admin Command Center Statistics
@app.get("/admin/dashboard-stats")
async def get_dashboard_stats():
    conn = get_db_connection()
    reports_count = conn.execute("SELECT COUNT(*) FROM reports WHERE status = 'Pending'").fetchone()[0]
    events_count = conn.execute("SELECT COUNT(*) FROM event_registrations").fetchone()[0]
    
    fest_total = conn.execute("SELECT COUNT(*) FROM fest_registrations").fetchone()[0]
    fest_upi = conn.execute("SELECT COUNT(*) FROM fest_registrations WHERE payment_type IN ('PhonePe', 'GooglePay')").fetchone()[0]
    
    conn.close()
    return {
        "pending_reports": reports_count,
        "total_event_regs": events_count,
        "fest_total": fest_total,
        "fest_upi": fest_upi
    }
...
async def get_fest_registrations():
    conn = get_db_connection()
    regs = conn.execute("SELECT * FROM fest_registrations ORDER BY id DESC").fetchall()
    conn.close()
    return {"data": [dict(row) for row in regs]}
...
async def get_event_stats():
    ...
    stats = conn.execute('''
        ...
        FROM campus_events e LEFT JOIN event_registrations r ON e.title = r.event_title GROUP BY e.title
    ''').fetchall()
    details = conn.execute('''
        SELECT r.event_title, u.roll_number, u.name 
        FROM event_registrations r JOIN users u ON r.roll_number = u.roll_number
    ''').fetchall()
    conn.close()
